    ...
    def __init__(self, team: Team, save_results: bool = True, use_cache: bool = True):
        ...
        for year in range(self.current_year - 3, self.current_year):
            ...
            self.historical_data.loc[year, "int %"] = float(
                passing_row.find(attrs={"data-stat": "pass_int_perc"}).get_text()
            )
            self.historical_data.loc[year, "pass td %"] = float(
                passing_row.find(attrs={"data-stat": "pass_td_perc"}).get_text()
            )
            self.historical_data.loc[year, "comp %"] = float(
                passing_row.find(attrs={"data-stat": "pass_cmp_perc"}).get_text()
            )
...
class SkillPlayer:
    ...
    def __init__(self, team: Team, pos: str):
        ...
        for year in range(self.current_year - 3, self.current_year):
            ...
            current_stat = rushing_row.find(
                attrs={"data-stat": "rush_yds_per_att"}
            ).get_text()
            if current_stat == "":
